////////////////////////// PARTICLE ENGINE ////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////
var canvaz = document.getElementById("projector");
if(canvaz){
...
function getMenuPos(){
	...
	if(headerPos.top==0){
		if(nav.classList.contains('white-bg')){
			nav.classList.remove('white-bg');
		}
		nav.classList.add("none-white-bg");
	}else {
		if(nav.classList.contains('none-white-bg')){
			nav.classList.remove('none-white-bg');
		}
...
// function init(){
// 	window.onscroll = function(blockClass) {
// 		var block = getElementsByClassName("ani-block");
...
if(window.location == "http://obsled.tech/")
{

function hideBlock() {
	var block = document.getElementsByClassName("ani-block");
	for(var i=0;i<block.length;i++){
		block[i].classList.add("hideme");
	}
	return block;
}
...
function blockPos() {
	var block = document.getElementsByClassName("ani-block");
	var curBlockCoords = block[0].getBoundingClientRect();

	var windowHeight = document.documentElement.clientHeight;

  	// верхняя граница elem в пределах видимости ИЛИ нижняя граница видима
  	var topVisible = (windowHeight - curBlockCoords.top) > 100 && curBlockCoords.top < windowHeight && curBlockCoords.bottom > 0;

  	return topVisible;
}

function addRemove(block,addCl,removeCl){
			block.classList.add(addCl);
			block.classList.remove(removeCl);
		}

function showVisible() {
	var visibilityVar = blockPos();
	if(visibilityVar === true) {
		var block = document.getElementsByClassName("ani-block");
		for(var i=0;i<block.length;i++){
			setTimeout(addRemove, 300 * i, block[i],"showme","hideme");
		}
	}
	else {
		var block = document.getElementsByClassName("ani-block");
		for(var i=0;i<block.length;i++){
			addRemove(block[i],"hideme","showme");
		}
	}
	}
}
...
/*function smallHeader() {
	var header = document.getElementsByClassName("head-container")[0];
	header.style.cssText="min-height: 250px !important; height: 250px;";
}*/

/*function cntForm () {
	var siteform = document.getElementById("cntform");
	var body = document.getElementsByTagName("body")[0];
	var modwin = document.createElement("div");
	modwin.setAttribute("id", "form-wrapper");
	modwin.style.display = "none";
	body.appendChild(modwin);
	modwin.appendChild(siteform);
}
...
function orderClick(e) {
	...
	cntForm();
	var modwin = document.getElementById("form-wrapper");
	console.log("pageYOffset:" + pageYOffset + "posit.top:" + posit.top);
	modwin.style.top = window.pageYOffset + "px";
	var siteform = document.getElementById("cntform");
	modwin.style.display = "block";
	siteform.style.display = "block";
	var topic = document.getElementById("mess-topic");
	topic.value = data;

}

function init() {
	var buttons = document.getElementsByClassName("header-price-button");
	console.log(buttons);
	for(var i=0;i<buttons.length;i++){
		buttons[i].onclick = orderClick;
	}
	
}*/

$(".header-price-button, .bottom-order, .form-call, .order-button").on("click", function(e){
	e.preventDefault();
	var topic = this.getAttribute("data-href");
	console.log(topic);
	var topicForm = document.getElementById("mess-topic");
